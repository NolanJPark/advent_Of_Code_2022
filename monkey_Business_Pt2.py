...
# Variable m will be a list that'll hold the monkey classes and product is how much the worry level will be % by
m = []
product = 1
while 1:
    s = f.readline()
    ...
    if s[0:6] == "Monkey":
        ...
        op = s[23: len(s)]
        s = f.readline()
        # Saves the int that the items worry level will be divided by and multiplies product by it
        test = int(s[21:len(s)])
        product *= test
        s = f.readline()
        ...
        for i in range(len(m[n].items)-1, -1, -1):
            ...
            if type == "*":
                ...
                if num == "old":
                    ...
                    x += int(num)
            # Mods the level of worry by product and saves it
            x = x % product
...
print("The level of monkey business after 10000 rounds is", nums[0]*nums[1])